...
dvai-bridge runs LLM inference on-device via Apple's GPU and Neural
...
- **Apple Silicon only at runtime.** MLX uses Metal Performance Shaders.
  iOS Simulator on Intel Macs has no MLX device. Real iOS devices and
...
- **SwiftPM only.** `mlx-swift-lm`'s transitive Swift dependencies
  ...
  should pick `.llama` or `.coreml`.
- **No embeddings.** The MLX handler returns HTTP 501 on
  `/v1/embeddings`. Use `.llama` with `embeddingMode: true` if you need
  embeddings.
...
Then point your OpenAI client at the returned `baseUrl` — same as any
other backend. The Capacitor MLX plugin is iOS-only. Selecting `mlx` on
Android throws an `iOS-only` error.

## Picking a model

Look under [`mlx-community`](https://huggingface.co/mlx-community) on
HuggingFace. The reference 4-bit Llama-3.2-1B is
...
on a Python host. Publish the result to HuggingFace and load it from
your dvai-bridge app.

## Multi-turn vs stateless

dvai-bridge's HTTP surface is stateless. Each `/v1/chat/completions`
request carries the entire message history. The MLX backend's underlying
[`ChatSession`](https://github.com/ml-explore/mlx-swift-lm/blob/main/Libraries/MLXLMCommon/ChatSession.swift)
*can* keep conversational state internally for higher throughput, but
that would conflict with our stateless HTTP semantics. The current
handler flattens the incoming `messages` array into a single prompt on
every request and re-applies the model's chat template via
...
- **Concurrency.** `ChatSession` is not thread-safe. The current handler
  ...
  workloads queue. Phase 3D may add a session pool.
...
- **First-run download is on the network.** Plan for a first-launch
  ...
  downloads — a Phase 3D follow-up.
