...
You imported `@dvai-bridge/core` in a Node script, Next.js server
route, serverless function, or Electron main process. Here's the path
from zero to a license-enforced production build.

## TL;DR

Place `dvai-license.jwt` at your project root — next to `package.json`.
The SDK reads it on `initialize()`. Production throws
`LicenseRequiredError` if the file is missing or invalid.
`NODE_ENV=development` and `NODE_ENV=test` skip the check.
...
secret — a Kubernetes secret, a Docker `--secret`. The SDK reads the
file once at startup. Never touches it again.
...
overrides every dev-mode signal. The SDK throws if the license is
missing.

## Audience binding for servers

Server-side Node has no browser hostname. Tell the SDK which `aud`
claim to match:
...
claim contains `"*"` — the any-host wildcard. It refuses ones bound to
specific domains. Most commercial licenses ship `"*"` and this works
out of the box. Bind explicitly with `DVAI_AUDIENCE` for stricter
checks.
