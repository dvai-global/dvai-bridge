...
Multi-gigabyte LLM weights can't ship inside an iOS or Android binary.
...
This page covers the surrounding decisions — where to host weights, how
to compute checksums, what the first-run UX looks like, and what to do
for multi-file (vision) models.
...
**For first launch:** pin a HuggingFace LFS revision via the
`resolve/<commit-sha>/` URL form. Move to your own bucket when beta
users hit rate-limit complaints.

When pinning HF, use the `resolve/main/<file>` form — not
`blob/main/<file>`. The blob form returns an HTML page, not raw bytes.

## Computing sha256

The downloader **requires** a sha256. There is no "trust me" mode.
Compute it once when you publish the file, hard-code the lowercase hex
digest in your app, and ship.
...
If the digest mismatches at download time, the helper deletes the
partial and destination files and throws. Retry-friendly. No half-bad
cache to clean up by hand.

## First-run UX

The first download dominates first-launch time-to-value for any local
AI app. Three things matter.

1. **Show progress — bytes and percentage.** A multi-GB download
   without feedback feels broken even when it isn't.
2. **Allow cancel.** Killing the process during download is fine. The
   `.partial` file stays in place, and the next attempt resumes via
   HTTP `Range`.
3. **Resume on app restart.** No extra code: `downloadModel()`
...
Progress events are throttled to ~10/sec — avoids pegging the JS thread
on a fast wifi link.

## Bundling small models in `public/`

For models small enough to ship inside the app bundle, drop them in
`public/models/<file>` and read the on-device path at runtime.
...
Bundle files can't be `mmap`'d directly on Android — Capacitor copies
...
Track aggregate progress by summing both files' `bytesTotal` ahead of
time and summing `bytesReceived` across the two phases. The default
progress listener fires for whichever download is currently active.
...
`hfToken` should be a **read-only**, fine-grained token scoped to the
specific repo. Never embed your personal access token in app code —
fetch it from your own backend at runtime, ideally exchanged from the
user's account. Treat its compromise as inevitable.
...
If `getFreeDiskSpace` isn't available in your Capacitor version, fall
back to a `statvfs`-style native shim — or just attempt the download
and surface the platform's `ENOSPC` / disk-full error.

## Cache management

Three helpers cover the cache surface. Everything else is your policy.
...
their device fills up. Provide an in-app "manage downloaded models" UI
that lists `listCachedModels()` and lets the user prune.
...
- Outbound traffic at download time reveals which model the user is
  fetching, plus the user's IP, to the host (HF, S3, your CDN).
- After download completes, the bridge generates no further outbound
  traffic.
- Cached weights live under app-private storage.
  ...
  - **Android** — `<filesDir>/dvai-models/`, auto-backed up only if
    your app opts in via `android:allowBackup`.
- Neither directory is reachable by other apps on the device.

If your app's privacy policy claims "no data leaves the device,"
remember — the model-fetch call is data leaving the device, even if the
payload is inert. Document it.

## See also

- [Capacitor quickstart](./quickstart-capacitor.md) — first-run code.
- [Multimodal](./multimodal.md) — content parts requiring vision or
  audio models often involve multi-file downloads.
