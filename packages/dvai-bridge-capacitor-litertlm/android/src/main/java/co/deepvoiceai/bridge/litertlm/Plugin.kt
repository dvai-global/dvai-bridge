...
@CapacitorPlugin(name = "DVAIBridgeLiteRTLM")
class DVAIBridgeLiteRTLMPlugin : Plugin() {
    private val state = LiteRTPluginState()
    ...
    fun start(call: PluginCall) {
        scope.launch {
            ...
            try {
                val result = state.start(call.data.toAnyMap())
    ...
    // For the LiteRT-LM backend, model lifecycle is the user's job (we don't
    ...
    fun downloadModel(call: PluginCall) {
        call.reject(
            "downloadModel is not implemented for LiteRT-LM backend in Phase 1. " +
                "Place .litertlm files in the app's filesDir manually."
        )
    }
    ...
    fun deleteCachedModel(call: PluginCall) {
        call.reject("deleteCachedModel is not implemented for LiteRT-LM backend in Phase 1.")
    }

    @PluginMethod
    fun cacheDir(call: PluginCall) {
        call.reject("cacheDir is not implemented for LiteRT-LM backend in Phase 1.")
    }
