...
/// <summary>
/// Cross-platform <see cref="INativeBridge"/> backed by ONNX Runtime + GenAI.
/// Resolved at runtime by <c>PlatformBridgeFactory</c> when a consumer
/// requests <see cref="BackendKind.Onnx"/> AND has installed the
/// <c>DVAIBridge.OnnxRuntime</c> NuGet (the assembly load is what makes
/// <c>Type.GetType(...)</c> succeed in the factory).
/// </summary>
internal sealed class OnnxNativeBridge : INativeBridge
{
    ...
    public OnnxNativeBridge() { }

    public async Task<BoundServer> StartAsync(StartOptions opts, CancellationToken ct)
    {
        if (_server is not null)
        {
            throw DVAIBridgeException.AlreadyStarted(BackendKind.Onnx, _server.BaseUrl);
        }

        if (opts.Backend != BackendKind.Onnx && opts.Backend != BackendKind.Auto)
        {
            throw DVAIBridgeException.BackendUnavailable(
                opts.Backend,
                $"OnnxNativeBridge only handles BackendKind.Onnx (got {opts.Backend}).");
        }

        if (string.IsNullOrEmpty(opts.ModelPath))
        {
            throw DVAIBridgeException.ConfigurationInvalid("ModelPath is required for the ONNX backend (directory containing model.onnx + genai_config.json + tokenizer.json).");
        }
        ...
        var modelId = opts.ModelId ?? Path.GetFileName(opts.ModelPath.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
        OnnxGenAIRunner runner;
        try
        {
            runner = await Task.Run(() => new OnnxGenAIRunner(opts.ModelPath, modelId), ct).ConfigureAwait(false);
        }
        ...
        var server = new OpenAIServer(runner, BackendKind.Onnx, opts.CorsOrigin,
        ...
        return new BoundServer(server.BaseUrl, server.Port, BackendKind.Onnx, modelId);
    }
    ...
    public Task<StatusInfo> GetStatusAsync(CancellationToken ct) =>
        Task.FromResult(_server is null
            ...
            : new StatusInfo(true, _server.BaseUrl, _server.Port, BackendKind.Onnx, _modelId));
    ...
    public Task<DownloadResult> DownloadModelAsync(DownloadOptions opts, CancellationToken ct) =>
        Task.FromException<DownloadResult>(DVAIBridgeException.ConfigurationInvalid(
            "ONNX backend doesn't ship a built-in downloader. Use the HuggingFace CLI " +
            "or the desktop slice's DownloadModelAsync to fetch model directories."));
    ...
    {
        private readonly OnnxNativeBridge _owner;
        private readonly Action<ProgressEvent> _handler;
        private bool _disposed;

        public Subscription(OnnxNativeBridge owner, Action<ProgressEvent> handler)
        {
            _owner = owner;
            _handler = handler;
        }
