...
describe('DvAI Config and Defaults', () => {
  it('should use default config values', () => {
    const dvai = new DvAI();
    expect(dvai.modelId).toBe('Qwen2.5-1.5B-Instruct-q4f16_1-MLC');
    expect(dvai.backend).toBe('webllm');
    expect(dvai.transformersModelId).toBe('Xenova/Qwen2.5-0.5B-Instruct');
  ...
  it('should throw on chatCompletion before initialization', async () => {
    const dvai = new DvAI();
    await expect(dvai.chatCompletion({ messages: [] })).rejects.toThrow('Backend not initialized');
  });
  ...
  it('should throw on runPipeline with webllm backend', async () => {
    ...
    await expect(dvai.runPipeline('test input')).rejects.toThrow('only available with the Transformers.js backend');
  });
});
