...
export class TransformersBackend {
  ...
  private resolvedDevice: "webgpu" | "cpu" = "cpu";
  ...
  async initialize(onProgress?: (info: any) => void): Promise<void> {
    ...
    if (this.device === "auto") {
      ...
      this.resolvedDevice = hasWebGPU ? "webgpu" : "cpu";
      console.log(`[DvAI/Transformers] Auto-detected device: ${this.resolvedDevice}`);
    } else {
    ...
    if (this.workerUrl && typeof Worker !== "undefined") {
      try {
        ...
        console.warn("[DvAI/Transformers] Worker initialization failed, falling back to main thread:", err);
        this.worker = null;
      }
    }
  ...
  private async initializeWithWorker(onProgress?: (info: any) => void): Promise<void> {
        ...
        switch (msg.type) {
          case "progress":
            if (onProgress) {
              ...
              const text = info.status === "progress"
          ...
          case "init_complete":
            ...
            console.log("[DvAI/Transformers] Initialized with Web Worker (main thread unblocked)");
            resolve();
            break;
          case "error":
            worker.removeEventListener("message", handleMessage);
  ...
  private sendWorkerRequest(type: string, data: Record<string, any> = {}): Promise<any> {
  ...
  private async initializeMainThread(onProgress?: (info: any) => void): Promise<void> {
    ...
    if (this.resolvedDevice === "cpu") {
      console.warn(
        "[DvAI/Transformers] Running on main thread with CPU — inference may block the UI. " +
        "Set `workerUrl` to a deployed transformers worker for better performance."
      );
    } else {
      console.log("[DvAI/Transformers] Initialized on main thread (WebGPU compute is async)");
    }
  }
  ...
  async runPipeline(inputs: any, options?: Record<string, any>): Promise<any> {
    ...
    if (!this.pipeline) throw new Error("Transformers.js pipeline not initialized");
    return this.withTimeout(
  ...
  async chatCompletion(requestBody: any): Promise<any> {
    ...
    if (this.usingWorker && this.worker) {
      ...
      result = await this.withTimeout(
        this.sendWorkerRequest("generate", { requestBody }),
        this.generationTimeout
      );
    } else if (this.pipeline) {
      // Run inference on main thread
      const messages = requestBody.messages || [];
      const maxTokens = requestBody.max_tokens ?? requestBody.max_completion_tokens ?? 256;
      const temperature = requestBody.temperature ?? 0.7;
      const topP = requestBody.top_p ?? 1.0;

      result = await this.withTimeout(
        this.pipeline(messages, {
